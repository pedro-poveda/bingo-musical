...
- **`auth.py`**: login de usuario Authorization Code + PKCE (sin client secret). Abre el navegador, recibe el código en un servidor HTTP local (Redirect URI registrada: `http://127.0.0.1:8888/callback`) y cachea access/refresh token en `~/.cache/bingo-musical/token.json` (permisos 600, carpeta 700) para no pedir login en cada ejecución. El servidor de callback (`_CallbackServer`/`_wait_for_callback`) ignora rutas distintas de `/callback` y peticiones con `state` incorrecto, y sigue esperando hasta `LOGIN_TIMEOUT`. Solo se invoca cuando Client Credentials no basta.
